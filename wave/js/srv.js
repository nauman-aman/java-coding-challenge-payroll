...
function($http, $log)	{
  ...
  var baseURL = 'http://localhost:8080/payroll'; // never change except for private unit tests
  ...
  this.buildURL = function(topic, instance_id, accessKey, context) {
	...
	this.generateReport	=	function(reportID) {
		...
		return $http.get(baseURL+'/generate?reportID='+reportID,
			JSON.stringify(),//fd, 
				{
					transformRequest: angular.identity,
					dataType: 'json',
	...
	this.getReportMetaData	=	function(reportID) {
		...
		return $http.get(baseURL+'/generate/metaData',
			JSON.stringify(),//fd, 
				{
					transformRequest: angular.identity,
					dataType: 'json',
